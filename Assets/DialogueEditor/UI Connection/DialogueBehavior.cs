...
public class DialogueBehavior : MonoBehaviour {
	...
	public DialogueObject GetNextDialogue(int optionId) {
		...
		//Comparison
		ComparativeNode node = null;
		foreach (var item in dialog.nodes)
		{
			if (item.id == optionId && item.nodeType == "Comparison")
				node = ((ComparativeNode)new ComparativeNode().SetNodeData(item.data));
			break;
		}
		if (node != null)
		{
			BaseNode resultNode = node.GetResult(parameters);
			if(resultNode!= null)
			{
				return GetNextDialogue(resultNode.id);
			}
		}
